function btest = bootstrap(test, boptions)
    ...
    function [statistic null] = newtest(X, Y, Z, k_X, k_Y, k_Z,k_Zs,options)
        stats = [];
        nulls = {};
     %   progress = ProgressMonitor(boptions.bootstrap_samples, 'Outer Bootstrap', 10);
        for b=1:boptions.bootstrap_samples%B=10
            [statistic null] = test(X, Y, Z, k_X, k_Y, k_Z, k_Zs, options);
            stats(end+1) = statistic;%MMD[i]
            nulls{end+1} = null;%inner_null[i,j]
